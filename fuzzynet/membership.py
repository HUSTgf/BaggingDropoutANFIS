...
from fuzzynet.cluster import ESSC
...
def make_gauss_mfs(sigma, mu_list):
    ...
    if isinstance(sigma, list):
        num = len(sigma)
        return [GaussMembFunc(mu_list[i], sigma[i]) for i in range(num)]
...
def make_mfs(x, num_mfs=2, num_out=1, mf_type='gaussmf'):
    ...
    for i in range(num_invars):
        sigma = ranges[i] / num_mfs  # ((num_mfs - 1)*2)
        ...
        if mf_type == 'gaussmf':
            ...
            b = sigma * 0.6
            c = mulist
            invars.append(('x{}'.format(i), make_bell_mfs(a, b, c)))
        elif mf_type == 'trimf':
            print('trimf')
            w = sigma * 6
            invars.append(('x{}'.format(i), make_tri_mfs(w, mulist)))
...
def make_mfs_clustering(x, num_clusters, num_out=1, mf_type='gaussmf'):
    num_invars = x.shape[1]
    invars = []
    outvars = ['y{}'.format(i) for i in range(num_out)]
    traindata = x.data.numpy()
    kmeans = KMeans(n_clusters=num_clusters, n_init=1, init='k-means++', tol=1e-6, max_iter=500, random_state=0).fit(
        traindata)

    for i in range(num_invars):
        mulist = kmeans.cluster_centers_[:, i].tolist()
        sigma = []
...
def make_mfs_clustering_fcm(x, num_clusters, num_out=1, mf_type='gaussmf'):
    num_invars = x.shape[1]
    invars = []
    outvars = ['y{}'.format(i) for i in range(num_out)]
    traindata = x.data.numpy()
    ...
    res = skfuzzy.cluster.cmeans(traindata, num_clusters, 2, error=0.0005, maxiter=500, init=None)
    cluster_centers = res[0]
    for i in range(num_invars):

        mulist = cluster_centers[:, i].tolist()
        sigma = []
        for j in range(num_clusters):
            sigma.append(0.2)
        if mf_type == 'gaussmf':
...
def make_mfs_clustering_essc(x, num_clusters, num_out=1, mf_type='gaussmf'):
    num_invars = x.shape[1]
    invars = []
    outvars = ['y{}'.format(i) for i in range(num_out)]
    traindata = x.data.numpy()
    essc = ESSC(n_cluster=num_clusters,m=2, scale=1, init='kmean', eta=0.1, tol_iter=500).fit(
        traindata)

    for i in range(num_invars):
        mulist = essc.center_[:, i].tolist()
        sigma = []
        for j in range(num_clusters):
            # inds = np.where(kmeans.labels_ == j)
            # tmp1 = traindata[inds,i]
            # tmp = np.std(tmp1)
            sigma.append(0.2)
        if mf_type == 'gaussmf':
            invars.append(('x{}'.format(i), make_gauss_mfs(sigma, mulist)))
